...
BEGIN
  ...
  BEGIN
    ...
    SELECT @id_est, @nom, @ape, 'FACTURA', f.id_factura, f.anio, f.mes,
           f.fecha_emision, f.fecha_vencimiento, f.monto_total, f.id_estado_pago
    FROM Factura f
    WHERE f.id_estudiante = @id_est;

    INSERT INTO #Pagos
    SELECT @id_est, @nom, @ape, 'CUOTA', c.id_cuota, NULL, c.mes,
           NULL, c.fecha_vencimiento, c.monto, c.id_estado_pago
    FROM Cuota c
...
BEGIN
  ...
  BEGIN
    ...
    SELECT @id_est, CONCAT(@ape, ', ', @nom),
           c.id_cuota, c.mes, c.fecha_vencimiento, c.monto, c.id_estado_pago
    FROM Cuota c
    WHERE c.id_estudiante = @id_est
      AND c.fecha_vencimiento < CAST(GETDATE() AS DATE)
      AND (c.id_estado_pago IS NULL OR c.id_estado_pago <> 2);
...
BEGIN
  ...
  BEGIN
    ...
    SELECT @id_curso, @nom_curso,
           (SELECT COUNT(*) FROM Inscripciones i WHERE i.id_curso = @id_curso);
...
BEGIN
  ...
  DECLARE cur CURSOR FOR
    SELECT DISTINCT COALESCE(id_estado_pago,'(sin estado)') FROM Factura;
  ...
  BEGIN
    ...
    SELECT @estado,
           ...
           SUM(monto_total)
    FROM Factura
    WHERE COALESCE(id_estado_pago,'(sin estado)') = @estado;
...
BEGIN
  ...
  DECLARE cur CURSOR FOR
    SELECT anio_carrera, porcentaje_interes FROM InteresporMora;
